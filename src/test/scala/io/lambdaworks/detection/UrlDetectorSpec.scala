...
final class UrlDetectorSpec extends AnyFlatSpec with Matchers {
  ...
  "URL Detector" should "extract the expected URLs with the default options" in {
    ...
    assertUrlExtraction(testCases, detector)
  }
  ...
  it should "handle invalid URLs gracefully without throwing exceptions" in {
    ...
    forAll(testCases) { (text, expectedUrls) =>
  ...
  private def testTable(cases: (String, Set[Url])*): TableFor2[String, Set[Url]] =
    Table(("text", "expectedUrls"), cases: _*)

  private def assertUrlExtraction(testCases: TableFor2[String, Set[Url]], detector: UrlDetector): Unit =
    forAll(testCases) { (text, expectedUrls) =>
      detector.extract(text).map(_.toString) shouldBe expectedUrls.map(_.toString)
    }
